import numpy as np
import random
from core.utils.noise import random_noise, rows_noise, blocks_noise
...
class InputHandle:
    def __init__(self, input_param):
        ...
        self.noise = input_param['noise']
        self.noise_ratio = input_param['noise_ratio']
        self.noise_size = input_param['noise_size']
        ...
        if self.noise is None:
            ...
            for i, item in enumerate(input_seq):
                if self.noise == 'random':
                    input_seq[i] = random_noise(item, self.input_length, corruption_ratio=self.noise_ratio)
                elif self.noise == 'rows':
                    input_seq[i] = rows_noise(item, self.input_length, corruption_ratio=self.noise_ratio)
                elif self.noise == 'blocks':
                    input_seq[i] = blocks_noise(item, self.input_length, corruption_ratio=self.noise_ratio, block_size=self.noise_size)
